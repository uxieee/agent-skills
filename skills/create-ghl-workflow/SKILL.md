...
- Auth is **`Authorization: Bearer <iframe JWT>`**. The documented `token-id` header returns **401** for the v2 user token — use Bearer.
- The JWT lives **~1 hour**. Capture it fresh right before a write burst, do all writes in one pass, and re-capture on 401.
- **Draft-first.** Build and verify everything as `draft`. Require explicit user confirmation before the publish PUT.
...
- `Bearer JWT` — DevTools → Network → any `backend.leadconnectorhq.com/workflow/...` request → copy the `Authorization` header value.
- `UID` — the JWT `authClassId` claim (decode the payload).
- `CID` — company id; read it off any row of `GET /workflow/{LOC}/list`.
