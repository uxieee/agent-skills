...
```
Authorization: Bearer <iframe JWT>      # NOT token-id (token-id 401s for the v2 user token)
channel: APP
source: WEB_USER
version: 2021-07-28
accept: application/json, text/plain, */*
content-type: application/json          # POST/PUT only
origin:  https://client-app-automation-workflows.leadconnectorhq.com   # writes
referer: https://client-app-automation-workflows.leadconnectorhq.com/  # writes
```

IDs: `LOC` location, `CID` company (read off an existing workflow), `UID` = JWT `authClassId` claim, `WID` workflow id (from create), `UUID` = a fresh uuid you generate.
